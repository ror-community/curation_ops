...
def retrieve_api(ror_id):
    api_url = "https://api.dev.ror.org/organizations/" + ror_id
...
def retrieve_from_ui(ror_id):
    ui_url = "https://dev.ror.org/" + ror_id
    href_value = "/" + ror_id
    driver.get(ui_url)
    sleep(2)
...
def search_name_api(org_name):
    api_url = "https://api.dev.ror.org/organizations?query=" + \
...
def search_name_ui(org_name):
    ui_url = "https://dev.ror.org/search?query=" + urllib.parse.quote(org_name)
    driver.get(ui_url)
    sleep(2)
    soup = BeautifulSoup(driver.page_source, features="html.parser")
