...
def compare_random_data_dump_production_api(release_ids):
	outfile = os.getcwd() + "/data_dump_production_discrepancies.csv"
	with open(DATA_DUMP_FILE_PATH, 'r+', encoding='utf8') as f_in:
		data_dump = json.load(f_in)
	minus_release_files = [
	...
	for record in random_data_dump_records:
		...
		print("Comparing data dump file and production api for", ror_id, "...")
		api_json = requests.get(api_url).json()
		if api_json == record:
			print("Data dump file matches production.\n")
		if api_json != record:
			print("Data dump file does not match production.\n")
			with open(outfile, 'a') as f_out:
				writer = csv.writer(f_out)
...
def compare_random_data_dump_production_api(release_ids):
	outfile = os.getcwd() + "/data_dump_production_api_discrepancies.csv"
	with open(DATA_DUMP_FILE_PATH, 'r+', encoding='utf8') as f_in:
		data_dump = json.load(f_in)
	minus_release_files = [
		record for record in data_dump if record["id"] not in release_ids]
