...
def check_in_json(f):
	...
	with open(f) as f_in:
		...
		for row in reader:
			json_file_path = curr_dir + row['json_file_name']
				...
				if row[field] != '':
					...
					if not isinstance(field_value, int) and ';' in field_value:
						...
						for value in field_value:
							if value not in ror_json.values():
								with open(outfile, 'a') as f_out:
									...
									writer.writerow([row['assigned_ror_id'], 'missing', field, value])
						...
						if field_value not in ror_json.values():
							with open(outfile, 'a') as f_out:
								writer = csv.writer(f_out)
								writer.writerow([row['assigned_ror_id'], 'missing', field, field_value])
					elif not isinstance(field_value, int) and field not in inverted_json[field_value].lower():
						with open(outfile, 'a') as f_out:
							writer = csv.writer(f_out)
							writer.writerow([row['assigned_ror_id'], 'transposition', field, field_value, inverted_json[field_value]])
