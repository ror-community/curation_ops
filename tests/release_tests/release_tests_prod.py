...
def retrieve_from_ui(ror_id):
    ui_url = f"https://dev.ror.org/{ror_id}"
    href_value = "/" + ror_id
    driver.get(ui_url)
    sleep(2)
...
def search_name_ui(org_name):
    ...
    ui_url = f'https://dev.ror.org/search?filter=status:active,status:inactive,status:withdrawn&query="{quoted_name}"'
    driver.get(ui_url)
    sleep(2)
    soup = BeautifulSoup(driver.page_source, features="html.parser")
...
def check_release_files(release_directory, all_ror_ids_file, release_tests_outfile, jsondiff_outfile):
    ...
    with open(release_tests_outfile, 'w') as f_out:
        ...
        writer.writerow(["ror_id", "org_name", "retrieve_check",
                         "compare_check", "search_name_api_check", "retrieve_ui_check", "search_name_check"])
    ...
    for file in json_files:
        ...
        search_name_api_check = search_name_api(
            org_name)
        retrieve_ui_check = retrieve_from_ui(ror_id)
        search_name_check = search_name_ui(org_name)
        with open(release_tests_outfile, 'a') as f_out:
            writer = csv.writer(f_out)
            writer.writerow([ror_id, org_name, retrieve_check, compare_check,
                             search_name_api_check, retrieve_ui_check, search_name_check])
    with open(all_ror_ids_file) as f_in:
        all_ror_ids = [line.strip()
                       for line in f_in if line.strip() not in ids_in_file]
        random_ids = random.sample(all_ror_ids, min(5, len(all_ror_ids)))
